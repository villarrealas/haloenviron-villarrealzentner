...
We determine our mass thresholds in such a way that no parameter exhibits
a trend that we can associate with resolution effects; we draw attention to the
downturn in \ref{fig:cnfwrelation} as an example of this, as the deviation from
the underlying monotonic trend suggests that this is a result of the box size
of the simulation and not a physically meaningful result. For ease of comparison
between halo definitions, we choose to use a single mass threshold for each
simulation box size and value of $\Delta$ chosen, rather than on a parameter by
...
\arz{What else could the thresholds be driven by?}
...
will study halo clustering as a function of satellite number.}  
...
otherwise the name seems arbitrary.}
...
suggesting that our method is robust with choice of summary statistic here?}
...
\begin{figure}
	...
	\caption{
The difference of the correlation function for only the top 20\% most concentrated halos 
and the bottom 20\% in concentration, normalized by the overall correlation function of 
the entire sample. From the top panel to the bottom panel, 
we show the results for the `low mass', `mid mass', and `high mass' cutoffs. 
...
sub-sample 20\% of all halos repeatedly.}
}
