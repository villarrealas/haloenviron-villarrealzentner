...
different halo definitions, utilizing the \citet{diemer_kravtsov15} simulations. We find that environmental dependencies
...
spherical halos. We find that for the properties tracing halo concentration, we can mitigate assembly bias at large scales through an appropriate choice of overdensity parameter. We also find that this result does not hold for parameters such as halo shape, halo spin, or halo satellite number. We suggest that this result is consistent with current models of halo formation and suggests the impact of large scale structure on these parameters.\arz{two or three sentence, relatively non-technical summary of our results 
should go here.}
...
\caption{•}
he relationship between the NFW concentration and halo mass for each of our simulations with $\Delta =200$. 
...
\arz{This figure is still way too busy to view. First, the legend covers some of the data. Second there are too many error 
bars. Try using error bars only for the L0250 simulation and just state in the caption that the dispersion in each simulation 
is similar in the regime in which the results are not compromised by resolution. Use a different line type for each line so that this 
can make sense in black and white if necessary. Lastly, is there some way to make the thresholds 
stand out even more? Perhaps they will stand out more when there is only one set of error bars? You will have to play with this. 
As far as the caption, this caption is way too brief. It should describe each line and it should state what the error bars are 
showing. Please make sure this and subsequent captions are sufficiently informative such that the figures are self contained. 
I will take a stab at this caption so that you can see what I mean.}
...
Each simulation is subject to resolution limitations at different halo masses. We show with thick vertical lines 
the minimum $M_{200}$ mass thresholds that we adopt in our analyses using the same color code as 
the concentration-mass relations, going from \simA \ to \simC \ from left to right. Note the deviation from a monotonic trend as a result of resolution effects. \arz{Same comments as 
for Fig.~\ref{fig:cnfwrelation}.}
}
...
concentration parameters. The white regions indicate where no halos hold these values. The scatter on this relationship ranges from 5\% for intermediate concentration values, to a high of 13\% at high masses. \arz{I don't know what the black dashed line shows, 
so you have to specify it.}.\asv{Currently I do not know what the black dashed line shows. I suspect it is from an old iteration of the plot - checking now.}
\arz{Previous caption was STILL not sufficiently specific. Consider carefully what I have 
written here as an example of a sufficiently specific caption.}
}
...
\caption{
\arz{All the same comments as for Fig.~\ref{fig:cnfwrelation}. 
In addition, I was under the impression that you were going to 
select thresholds that guaranteed that the $s(M)$ relation 
is monotonic? Am I mistaken?}\asv{Correct, reruns are currently at work due to shift in halotools}
...
robust measurement of halo shape. \asv{Tried to explain the difference in cuts
as best I could in the caption. Need to look up exact shape parameter cuts, but
I suspect it will be similar progression as previously, but starting at
2e12 for L0125, 2e13 for L0250, and maybe 3e13 for L0500. Really going to be
cutting down on the total number of halos, but it needs to be done.}
}
\vspace*{8pt}
...
{L0125-shape} & {N/A} & $2 \times 10^{12}$ & $2 \times 10^{12}$ & $2 \times 10^{12}$ & $2 \times 10^{12}$ & $2 \times 10^{12}$ \\
{L0250-gen} & {N/A} & $7 \times 10^{11}$ & $8 \times 10^{11}$ & $9 \times 10^{11}$ & $1.5 \times 10^{12}$ & {N/A} \\
{L0250-shape} & {N/A} & $8 \times 10^{12}$ & $8 \times 10^{12}$ & $8 \times 10^{12}$ & $8 \times 10^{12}$ & {N/A} \\
{L0500-gen} & $3 \times 10^{12}$ & $4 \times 10^{12}$ & $5 \times 10^{12}$ & $6 \times 10^{12}$ & $7 \times 10^{12}$ & {N/A} \\
{L0500-shape} & $4 \times 10^{13}$ & $4 \times 10^{13}$ & $4 \times 10^{13}$ & $4 \times 10^{13}$ & $4 \times 10^{13}$ & {N/A} \\
\hline
\hline
\end{tabular}
...
\simA \ (\simB /\simC) data set utilizing the \simA -gen (\simB -gen/\simC -gen) mass cutoffs. Note
...
\begin{figure}
	...
	the $\Delta = 200$ and $\Delta = 10$ catalogs for \simA. The blue (red) line shows the mean value and scatter of
	the	mean for the $\Delta = 200$ ($\Delta = 70$) catalog. Note that there is no statistically significant
...
\simA \ (\simB /\simC) data set utilizing the \simA -shape (\simB -shape/\simC -shape) mass cutoffs. Note
that the top panel is the only one where exploring the extreme regime of $\Delta = 10$ is
necessary, while the bottom panel is the only one where the exploration of $\Delta = 340$
is necessary. The shaded bands represent 2-sigma confidence regions generated by randomization of the marks.}\asv{Some of this might be driven by not having
an aggressive enough mass cutoff, but if this trend continues, it is difficult
to write the ordering of the $\Delta$ values.}
	\label{fig:cc_mcf_s}
\end{figure}
%--------------------------------------------
...
\begin{figure*}
	...
	\caption{A $20 \hMpc$ deep cut of \simB \ along the z-axis. This zoom-in demonstrates the process that decreases the shape parameter as a function of clustering. The radius of each circle corresponds directly with the the value of $r_{\Delta}$ determined for each host halo. Filled circles use the $\Delta = 200$ catalog and unfilled circles use the $\Delta = 10$ catalog in order to make the effect more visible. Color scale refers to the shape mark, normalized by halos of the same mass where red (blue) corresponds to higher (lower) values of halo shape compared against halos of the same mass.}
