...
Dark matter halo clustering depends not only on halo mass, but also on other properties such as concentration and shape. This phenomenon is known broadly as {\em assembly bias}. We explore the dependence assembly bias on halo definition, parametrized by spherical overdensity parameter, $\Delta$. We summarize the strength of concentration-, shape-, and spin-dependent halo clustering as a function of both halo mass and halo definition. Concentration-dependent clustering depends strongly on halo mass at all $\Delta$. For conventional halo definitions, concentration-dependent halo clustering at low mass is driven by a population of haloes that is altered by interactions with neighbouring haloes. Concentration-dependent clustering can be greatly reduced by adopting a mass-dependent halo definition with $\Delta \sim 20-40\mathrm{m}$ for haloes with $M_{200\mathrm{m}} \lesssim 10^{12}\, h^{-1}\mathrm{M}_{\odot}$. Defining low-mass haloes to have larger radii (smaller $\Delta$) mitigates assembly bias by subsuming altered,  so-called {\em backsplash} haloes into now larger host haloes. At higher masses ($M_{200\mathrm{m}} \gtrsim 10^{13}\, h^{-1}\mathrm{M}_{\odot}$) larger overdensities, $\Delta \gtrsim 600\mathrm{m}$, are necessary. Shape- and spin-dependent clustering are significant for all halo definitions that we explore and exhibit a relatively weaker mass dependence. Generally, both the strength and the sense of halo assembly bias depend on halo definition, varying significantly even among common definitions. We identify no halo definition that mitigates all manifestations of assembly bias. A halo definition that mitigates assembly bias based on one halo property (e.g., concentration) must be mass dependent. The halo definitions that best mitigate concentration-dependent halo clustering do not coincide with the expected {\em average} splashback radii at fixed halo mass.
...
X-ray studies of clusters use higher overdensities to isolate the regions within which they have data, with overdensities reaching as high as $\Delta \approx 7800$m ($\approx 2500$c). \arz{Via DC - Is it possible to add a few examples for each Delta?}
...
Fig.~\ref{fig:cc_mcf_cV} is a similar plot for the MCF of the velocity-ratio concentration, $c_{\mathrm{V}}$. This figure exhibits qualitatively very similar features to Fig.~\ref{fig:cc_mcf_cnfw}, a fact that is not surprising given that we already know that $c_{\mathrm{NFW}}$ and $c_{\mathrm{V}}$ quantify largely redundant information about their haloes. This demonstrates that our results regarding the halo boundary dependence (or $\Delta$ dependence) of halo clustering is not driven by details of a fit of the density profile to the NFW functional form, which is an increasingly poor description of the halo profiles for halo-centric distances significantly larger than $R_{200\text{m}}$. {\bf [DC: is there a citation or demonstration of this?]}
\fvdb{I suggest adding here something along the lines of: "As a consequence, $c_{\mathrm NFW}$
becomes more and more noisy as $\Delta$ decreases. The alert reader may wonder whether this
might explain why smaller values for $\Delta$ reveal less assembly bias; after all, if the mark
is purely stochastic one expects that the MCF is equal to zero. However, as we demonstrate in
\S\ref{section:halodefmit}, this is not the case, and the reduction in the MCF with decreasing $\Delta$ truly reflects a weakening of the strength of assembly bias."}
...
\fvdb{I would like to suggest one additional figure. Show the one-point distribution functions
for the marks for the different values of $\Delta$. That way the reader can see how changing
$\Delta$ changes the characteristic values of the marks. Personally I am curious to see how
for instance the spin parameter distribution depends on $\Delta$. This could be a simple plot
with four panels (one for each of the four marks used in this paper), and each panel shows 
PDFs for the same 6 values of  $\Delta$ as used in the other Figs.}
...
very weak concentration-dependent clustering.\\
    ...
    \item Halo shape-dependent clustering is significant over a wide range of halo definitions, but does not exhibit a strong mass dependence for any particular halo definition (Fig.~\ref{fig:cc_mcf_s} and Fig.~\ref{fig:biascompare}). Mitigating shape-dependent halo clustering requires an extreme halo definition, with $\Delta \lesssim 10$m.\fvdb{a picky referee, aka yours truly, might argue that you have not demonstrated this. Your results are suggestive of this, but unless you have explicitely demonstrated this, I suggest you rewrite this...something like; Shape dependent halo clustering cannot be mitigated with any of the halo definitions examined here $20\text{m} \leq \Delta \leq 625\text{m}$, but there is a trend that its strength weakens as $\Delta$ is reduced.}\\
