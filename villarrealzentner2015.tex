...
\begin{figure*}
	...
	Each panel contains the relative bias of the 20\% highest marked sample to the full sample, plotted against halo mass measured at the halo radius. The marks of interest are labelled above each plot: clockwise from the top-left these are NFW concentration, velocity ratio concentration, spin parameter, and halo shape. The halo mass binning is defined in full detail in the text. Halo definitions descend from the largest definition to lowest definition, from dark to light in color. The dark blue (light blue) line uses a halo definition drawn from $\Delta = 6250$ ($\Delta = 50$). The solid (dot-dashed, dashed) lines use host halos from the \simA (\simB,\simC) catalogs. The
	...
	shows where there is no detected assembly bias driven by mark of interest.}
