...
$\Delta=60$ catalog. Those host halos that {\em do not} exist in the $\Delta=60$ catalog are precisely those halos that have been subsumed by the larger halo radii in this case and thus been classified as subhalos. This achieves the goal of redefining halos by the lower $\Delta$ threshold ($\Delta=60$ in this example), but eliminates that fact that the properties we are studying are also defined differently in the lower $\Delta$ catalog, because we are studying their clustering as a function of the properties defined in the higher $\Delta$ ($\Delta=200$ in this example) catalog. We refer to samples constructed in this way as ``matched" halo samples between two values of threshold overdensity $\Delta$. 
...
In comparing a pair of dashed and solid lines at the same $\Delta$ threshold in 
Fig.~\ref{fig:hvm_mcf_cnfw}, one is comparing the MCF of all halos at that $\Delta$ threshold, to the MCF of only those halos identified in the lower threshold samples. 
The difference between a pair of solid and dashed lines at fixed $\Delta$ is caused  entirely by the exclusion of host halos due to a change in halo definition. Notice that these solid lines show greatly reduced concentration-dependent clustering. This suggests that much of assembly bias is driven by the interactions of nearby halos and that subsuming a larger region into a halo to accommodate these interaction may be well motivated and useful. In each case, there is some residual assembly bias exhibited by the solid lines which are not all consistent with zero. The remaining component of the reduction of assembly bias signal is driven by an increase in noise or some more fundamental difference in how the marks are measured or mass is assigned. We look forward to probing the impact of these effects in the future.
...
filaments and satellite galaxy position \citep{tempel_etal15, velliscig_etal15, shao_etal16}.
