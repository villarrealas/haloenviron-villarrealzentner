...
\begin{abstract}
%% Abstract goes here
Recent work has shown the importance of environment to the properties of dark matter halos. This brings conflict to standard implementations of the halo model and excursion set theory which assume that the properties of a population within the halo is determined by the mass of the halo alone. We seek to find a definition of the size of a halo that allows us to circumvent these environmental effects. We analyze the dependence on environment of our properties using the method of marked correlation functions for several different halo definitions, utilizing the Max and Consuelo simulations. We find that environmental dependencies are dramatically different as we vary the definition of the halo radius in terms of the overdensity parameter $\Delta$. We show that not every halo property converges to our confidence region in the same manner and contemplate where our results may prove useful and what might be implied by the other cases.
\end{abstract}
...
In order to study the effects of halo redefinition, we make extensive use of two simulation suites. The Max simulation uses Zel'dovich approximation initial conditions with $\Om = 0.3036$, $\Ol = 0.6964$ and  $h_0 =  0.6814$. The simulation box has a comoving size of $200 \hMpc$ and a particle mass of $2.18396 \times 10^9 \hMsun$, with a total of $768^3$ particles. This allows us to make relevant analysis on structure formation inside of our box on scales up to about $30 \hMpc$. We also analyze the Consuelo simulation from the Large Suite of Dark Matter Simulations (LasDamas) (McBride et al, in prep? I literally cannot find this paper despite it being in prep since 2012!) :which ran with a cosmology with $\Om = 0.25$, $\Ol = 0.75$, and $h_o = 0.7$. This simulation box has a comoving size of $420 \hMpc$ and a particle mass of $1.87\times 10^9 \hMsun$, with a total of $1400^3$ particles.
...
\beq
	c_{\mathrm{vratio}} = \frac{v_{\mathrm{max}}}{v_{\Delta}} ; \ \ c_{\mathrm{NFW}} = \frac{\Rdel}{r_\mathrm{s}}; \ \ s = \frac{c}{a} \ \
\eeq
...
There are several well understood effects in our simulation that must be accounted for before we can draw any conclusions. The first is the matter of simulation resolution. In each of our generated halo catalogs, we can anticipate finding artificial halos. While existing under our constraint of having only a set overdensity, they will have properties that are contrary to the well known trends that have been shown in prior works, such as concentration decreasing as a function of halo mass \citep{wechsler06}. As shown in Figure 2, we can roughly identify the region in which these artificial halos become significant by looking for the turnover in the plot. In order to avoid our halo finding statistics being thrown off by these artificial halos, we set a lower mass threshold, limiting the sample to only those halos that are physically significant. We choose to use the higher mass constraint between the Max and Consuelo simulation boxes for consistency.

\begin{figure}
	\centering
		\includegraphics[width=.5\textwidth]{masscut_d200.pdf}
	\caption{An example of the chosen lower limit on halo mass for our sample is marked as a black line, for the $\Delta = 200$ case. The shaded blue region depicts the spread on the power in the concentration-mass relationship proposed by \citet{duffy08}. At lower mass, halos are ill defined due to resolution limits. The shaded region illustrates the implied two-sigma band of \citet{duffy08} mass-concentration relation.}
\end{figure}
...
%% for each of the following results, should put Max and LasDamas results side by side
\begin{figure}
	\centering
	\subfloat[Consuelo]{{\includegraphics[width=.5\textwidth]{ld_cf_compare_z00.pdf}} }
	\qquad
	\subfloat[Max]{{\includegraphics[width=.5\textwidth]{max_cf_compare_z00.pdf}} }
...
\begin{figure}
	\centering
	\subfloat[Consuelo]{{\includegraphics[width=.5\textwidth]{ld_mcf_cnfw_z00.pdf}} }
	\qquad
	\subfloat[Max]{{\includegraphics[width=.5\textwidth]{max_mcf_cnfw_z00.pdf}} }
...
\begin{figure}
	\centering
	\subfloat[Consuelo]{{\includegraphics[width=.5\textwidth]{ld_mcf_vrat_z00.pdf}} }
	\qquad
	\subfloat[Max]{{\includegraphics[width=.5\textwidth]{max_mcf_vrat_z00.pdf}} }
...
\begin{figure}
	\centering
	\subfloat[Consuelo]{{\includegraphics[width=.5\textwidth]{ld_mcf_s_z00.pdf}} }
	\qquad
	\subfloat[Max]{{\includegraphics[width=.5\textwidth]{max_mcf_s_z00.pdf}} }
...
Figure 6 demonstrates a case in which the halo property does not have environmental dependence removed under our simple method of halo redefinition. In particular, with decreasing values of $\Delta$, we only serve to increase the strength of this environmental dependence. This result seems to indicate a process occurring to drive the shape of the halo is on scales that are significantly larger than the size of the halo, such that our method will not be able to encompass these effects. A similar result is located in Figure 7, in which the satellite number mark experiences the exact same effect.

\begin{figure}
	\centering
	\subfloat[Consuelo]{{\includegraphics[width=.5\textwidth]{ld_mcf_ns_z00.pdf}} }
	\qquad
	\subfloat[Max]{{\includegraphics[width=.5\textwidth]{max_mcf_ns_z00.pdf}} }
